const { NekoGirlStateMachine, Emotion, Animation, LookDirections } = require('../NekoGirlStateMachine');
...
describe('NekoGirlStateMachine', () => {
  ...
  describe('setEmotion', () => {
    it('should set emotion and emit event', () => {
      ...
      expect(emotionSpy).toHaveBeenCalledWith('emotion_change', { emotion: Emotion.HAPPY });
      expect(mockLogger.info).toHaveBeenCalledWith(`Emotion set to ${Emotion.HAPPY}`);
    });
  ...
  describe('setAnimation', () => {
    it('should set animation and emit event', () => {
      ...
      expect(animationSpy).toHaveBeenCalledWith('animation_change', { animation: Animation.DANCING });
      expect(mockLogger.info).toHaveBeenCalledWith(`Animation set to ${Animation.DANCING}`);
    });
  ...
  describe('setAudio', () => {
    it('should set audio URL and emit event', () => {
      ...
      expect(audioSpy).toHaveBeenCalledWith('audio_change', { audioUrl: testUrl });
      expect(mockLogger.info).toHaveBeenCalledWith(`Audio URL set to ${testUrl}`);
    });
  ...
  describe('resetActivityTimer', () => {
    ...
    it('should set new timer that calls resetToNeutral', () => {
      const resetToNeutralSpy = jest.spyOn(stateMachine, 'resetToNeutral');
  ...
  describe('resetToNeutral', () => {
    ...
    it('should emit state_reset event with current states', () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Reset to neutral due to inactivity');
    });
  });

  describe('Enum Validations', () => {
    it('should have all required Emotion states', () => {
      expect(Object.values(Emotion)).toContain('neutral');
      expect(Object.values(Emotion)).toContain('happy');
      expect(Object.values(Emotion)).toContain('sad');
      expect(Object.values(Emotion)).toContain('angry');
      expect(Object.values(Emotion)).toContain('surprised');
    });

    it('should have all required Animation states', () => {
      expect(Object.values(Animation)).toContain('none');
      expect(Object.values(Animation)).toContain('silly_dancing');
      expect(Object.values(Animation)).toContain('angry');
      expect(Object.values(Animation)).toContain('neutral');
      expect(Object.values(Animation)).toContain('look_left');
      expect(Object.values(Animation)).toContain('look_right');
    });

    it('should have all required LookDirections', () => {
      expect(Object.values(LookDirections)).toContain('forward');
      expect(Object.values(LookDirections)).toContain('up');
      expect(Object.values(LookDirections)).toContain('down');
      expect(Object.values(LookDirections)).toContain('left');
      expect(Object.values(LookDirections)).toContain('right');
    });
  });
  ...
  describe('Integration Tests', () => {
    it('should maintain state through multiple changes until timeout', () => {
      ...
      expect(stateMachine.currentEmotion).toBe(Emotion.HAPPY);
      expect(stateMachine.currentAnimation).toBe(Animation.DANCING);
      expect(stateMachine.currentAudio).toBe('http://example.com/audio.mp3');
